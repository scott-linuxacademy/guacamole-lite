...
class GuacdClient {
    ...
    sendHandshakeReply() {
        var connection = this.clientConnection.connectionSettings.connection;
        this.sendOpCode([
            'size',
            connection.width || '1024',
            connection.height || '768',
            connection.dpi || '96'
        ]);

        // TODO these need to be passable as options
        this.sendOpCode(['audio', 'audio/ogg']);
        this.sendOpCode(['video']);
        this.sendOpCode(['image', 'image/png', 'image/jpeg']);
        ...
        let connectionOptions = ['connect'];

        serverHandshake.forEach((attribute) => {
            let actual = attribute.replace(/^\d+\./, '');
            if (actual != 'args') {
                connectionOptions.push(this.getConnectionOption(actual));
            }
        });
